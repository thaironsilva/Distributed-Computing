...
/* Write Double to a stream socket */
int WriteDouble(TSocket sock, double *n) {
	write(sock, n, sizeof(double));
	return 0;
}

/* Read double from a stream socket */
int ReadDouble(TSocket sock, double *n) {
	read(sock, n, sizeof(double));
  return 0;
}
